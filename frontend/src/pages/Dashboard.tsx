...
import { DUMMY_SELLERS, MAX_DISPLAY_SELLERS, STATUS_COLORS } from '../constants';
...
const SellerCard: React.FC<{ seller: Seller }> = ({ seller }) => {
  return (
    <div className="bg-zinc-800 rounded-lg shadow-md p-6 mb-4 min-w-[250px]">
      <h3 className="text-xl font-semibold text-violet-400 mb-2">{seller.name}</h3>
      <p className="text-zinc-300">Emission: {seller.emissionAmount} tons</p>
      <p className="text-zinc-300">
        Verified: {seller.verifiedAmount !== null ? `${seller.verifiedAmount} tons` : 'Pending'}
      </p>
...
const Dashboard: React.FC = () => {
  ...
  const totalEmission = DUMMY_SELLERS.reduce((sum, seller) => sum + seller.emissionAmount, 0);
  const totalVerified = DUMMY_SELLERS.reduce((sum, seller) => sum + (seller.verifiedAmount || 0), 0);
  ...
  return (
    ...
    <Breadcrumb />
      <h2 className="text-3xl font-bold mb-6 text-zinc-100">Carbon Emission Dashboard</h2>
      
      {/* First Row: Seller Cards */}
      <div className="mb-8">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold text-zinc-200">Recent Sellers</h3>
          <Link 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-zinc-800 rounded-lg shadow-md p-6">
          ...
          <p className="text-zinc-300">Verification Rate: {((totalVerified / totalEmission) * 100).toFixed(2)}%</p>
        </div>
        
        {/* Recent Activities or News */}
        <div className="bg-zinc-800 rounded-lg shadow-md p-6">
          <h3 className="text-xl font-semibold mb-4 text-zinc-200">Recent Activities</h3>
          <ul className="text-zinc-300">
            <li className="mb-2">New seller "Green Tech" registered</li>
            <li className="mb-2">Verification completed for "Eco Solutions Inc."</li>
            <li className="mb-2">Carbon credits issued to "Sustainable Futures"</li>
          </ul>
        </div>
      </div>
    </div>
